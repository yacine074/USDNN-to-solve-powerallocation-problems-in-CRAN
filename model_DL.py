...
def archi_hlayers_choice(X_train, f_loss, f_metrics, f_activation1, f_activation2, f_activation3, LR, hidden_layer_nbr):
    
    opt = tf.keras.optimizers.Adam(learning_rate = LR)
    
    if hidden_layer_nbr == 4 : 
        inputs = Input(shape=(X_train.shape[1]))
        x = Dense(128, activation='relu')(inputs)
        x = Dense(256, activation='relu')(x)
        x = Dense(256, activation='relu')(x)
...
def archi_neurons_choice(X_train, f_loss, f_metrics, f_activation1, f_activation2, f_activation3, LR, M):
    opt = tf.keras.optimizers.Adam(learning_rate = LR)
